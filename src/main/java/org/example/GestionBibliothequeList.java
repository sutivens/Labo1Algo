...
public class GestionBibliothequeList {
    public static void main(String[] args) {
        boolean fin = false;
        BiblioList biblioList = new BiblioList();

        biblioList.ajouterOuvrage(new Livre("01/01/2023", "Auteur du livre", "Titre du livre", "Éditeur du livre"));
        biblioList.ajouterOuvrage(new CD("02/02/2023", "Titre du CD", "Auteur du CD"));
        biblioList.ajouterOuvrage(new Periodique("03/03/2023", "Nom", 1, "Mensuel"));
        ...
        while (!fin) {
            ...
            switch (choix) {
                ...
                case "2":
                    // Rechercher par côte
                    afficherRecherche(biblioList);
                    break;
                case "3":
                    // Ajouter un ouvrage
                    menuAjouter(biblioList);
                    break;
                case "4":
                    // Supprimer un ouvrage
                    menuSupprimer(biblioList);
                    break;
                case "0":
                    // Quitter
    ...
    public static void afficherRecherche(BiblioList biblioList) {
        Scanner scanner = new Scanner(System.in);

        try {
            System.out.print("Entrez la côte: ");
            int coteSaisi = Integer.parseInt(scanner.nextLine());
            Ouvrage ouvrage = biblioList.rechercherOuvrage(coteSaisi);
            if (ouvrage != null) {
    ...
    public static void menuAjouter(BiblioList biblioList) {
        Scanner scanner = new Scanner(System.in);
        ...
        if (ouvrage != null) {
            biblioList.ajouterOuvrage(ouvrage);
            System.out.println("Ouvrage ajouté!\n");
        }
    }

    public static void menuSupprimer(BiblioList biblioTab) {
        Scanner scanner = new Scanner(System.in);
