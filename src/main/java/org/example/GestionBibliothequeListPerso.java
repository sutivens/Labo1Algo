...
public class GestionBibliothequeListPerso {
    public static void main(String[] args) {
        boolean fin = false;
        BiblioListPerso biblioListPerso = new BiblioListPerso();

        biblioListPerso.ajouterOuvrage(new Livre("01/01/2023", "Auteur du livre", "Titre du livre", "Éditeur du livre"));
        biblioListPerso.ajouterOuvrage(new CD("02/02/2023", "Titre du CD", "Auteur du CD"));
        biblioListPerso.ajouterOuvrage(new Periodique("03/03/2023", "Nom", 1, "Mensuel"));
        ...
        while (!fin) {
            ...
            switch (choix) {
                ...
                case "2":
                    // Rechercher par côte
                    afficherRecherche(biblioListPerso);
                    break;
                case "3":
                    // Ajouter un ouvrage
                    menuAjouter(biblioListPerso);
                    break;
                case "4":
                    // Supprimer un ouvrage
                    menuSupprimer(biblioListPerso);
                    break;
                case "0":
                    // Quitter
    ...
    public static void afficherRecherche(BiblioListPerso biblioListPerso) {
        Scanner scanner = new Scanner(System.in);

        try {
            System.out.print("Entrez la côte: ");
            int coteSaisi = Integer.parseInt(scanner.nextLine());
            Ouvrage ouvrage = biblioListPerso.rechercherOuvrage(coteSaisi);
            if (ouvrage != null) {
    ...
    public static void menuAjouter(BiblioListPerso biblioListPerso) {
        Scanner scanner = new Scanner(System.in);
        ...
        if (ouvrage != null) {
            biblioListPerso.ajouterOuvrage(ouvrage);
            System.out.println("Ouvrage ajouté!\n");
        }
    }

    public static void menuSupprimer(BiblioListPerso biblioListPerso) {
        Scanner scanner = new Scanner(System.in);

        System.out.print("Veuillez saisir la côte de l'ouvrage à supprimer: ");
        try {
            int coteSaisi = Integer.parseInt(scanner.nextLine());
            biblioListPerso.supprimerOuvrage(coteSaisi);
        } catch (Exception e) {
            System.out.println("Erreur! Vous n'avez pas entré un nombre ou chiffres valides.\n");
        }
    }
